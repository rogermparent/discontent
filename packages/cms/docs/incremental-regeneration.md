...
**F33 — taxonomy as an engine kind (epic 24, `websites/recipe-website/docs/agent-taxonomy.md`).**
Every carrier of tags in the repo — recipes, demo notes, portfolio projects — folds its own
`string[]` its own way, or not at all, and none of them knows what a tag _means_. F33 makes the
vocabulary a declaration on the content type: `taxonomies: [{name, field, version, slugOf?,
project?, terms?}]`. From it the engine derives two aggregates per taxonomy through the existing
kind — `<name>s` (`Array<{slug, label, count}>`, sorted by slug, first label wins) and
`by-<name>` (`Record<slug, {label, items}>`, items newest-first, `project` deciding the row) —
appended after the type's declared aggregates by `aggregatesOf(config)`, which is what
`updateAggregates` and `derivedTagsOf` now iterate. Names keep the strings the recipe site already
uses (`tags`, `by-tag`), so cache tags and LMDB directories do not move when the site adopts it; the
terms _shape_ moves, so the site's version bumps at adoption. The spec version stored is
`<engine fold version>.<taxonomy version>`, so a fold edit in the engine bumps one constant.

The second half is the **term record**: `createTermContentType({taxonomy, directory})` is an
ordinary `ContentTypeConfig` — `term.json` per slug with `{label, date, description?, image?,
parent?}` — whose one novelty is a **self-referencing** reference edge: `references` and
`referencedBy` both point at the config itself so a parent rename rewrites its children and a
child's index value borrows `parentLabel`. Whether `updateDependents` survives dependent = self is
the phase's open question (24a proves it on the demo or records why not). A `tree` aggregate folds
`{label, parent?, children}` per slug from the term index alone; a visited set makes a hand-edited
cycle terminate. Term identity is the slug of the normalised label, so carriers keep storing
strings and nothing on disk changes — the hybrid the epic doc's D3 argues for over materialised
paths or slug assignments (the latter would need F32).
