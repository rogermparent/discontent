...
> **Epic 23 (Recipe MCP) is in `agent-mcp.md`.** Rows are struck here as
> that epic's phases pick them up; the struck rows below name the phase.
...
| --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- |
| ~~`group set-image` / a group **update** seat in the curation layer + CLI~~ | ~~`group create --image-url` is create-only; an existing group takes an image only through the editor form. The same seat would carry name/description/item edits from the CLI.~~ **Picked up by 23a** (`agent-mcp.md`, D4). | agent-curation Deferred (22h)         |
...
| ~~CLI `feature` / `unfeature` commands~~                                    | ~~Featuring a group (or recipe) is editor-only; the skill cannot finish a "make a collection and put it on the homepage" ask.~~ **Picked up by 23a** (`agent-mcp.md`, D5).                                                   | agent-curation Deferred (22g)         |
...
| **Trap: deleting a stacked PR's base branch closes its child**         | GitHub retargets a child PR only when the parent is merged _and_ its branch deleted through the merge UI; deleting the branch by API (`gh pr merge --delete-branch`, a remote branch delete) closes the child instead (#124 on 2026-09-10; recovered by recreating the branch at its old tip, reopening, `gh pr edit --base`, deleting again). Order for a stack: merge, retarget the child, then delete. | this landing                  |
| `lint` job scales with the `main` gap                                  | `lint.yml` runs `lint-staged --diff origin/main`, so every branch PR lints the whole `main`…branch diff (≈1200 files before promotion): eslint was SIGKILLed once on #136's run (runner memory), and a formatting slip anywhere in the gap fails every PR. Promotion shrinks the gap to nothing; a `--diff` against the PR base would keep it small for good.                                             | this landing (#136 lint run)  |
...
**After #133 (2026-09-10):** the run at the 22e merge (`dab0bea0`, run 34435970141) was green on all eight jobs — portfolio in 3.5 min, the recipe
shards in 5–8 min, so the 30-minute cancellations were entirely the failing
sign-ins' retries. Nothing from the first run is left open; the row below for
the `unix-dgram` install warning is the only residue.
