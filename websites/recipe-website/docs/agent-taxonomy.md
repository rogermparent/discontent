...
> merged (#143 → `main` `6709f4c8`, 2026-09-17); 24b is in progress in
> `agent/24b-taxonomy-adopt` (D5 decided below).** The previous
...
| 24b     | `agent/24b-taxonomy-adopt` ← `main`  | 🟡 in progress | Recipes → taxonomy (delete the pair, keep names, v2), readers, groups gain `tags` (types, index value, schemas, form, seat), portfolio declares + routes, `/tags` unions; all 15 recipe fixtures regenerated; **D5 decided** (M–L)                          |
...
**Current PR: 24b** — `agent/24b-taxonomy-adopt` off `main` at `6709f4c8`
(the #143 merge), worktree `.claude/worktrees/agent-24b`. D5 is decided
(option 2); the phase section below is the implementer's brief. **Next PR
after it: 24c** — term records + the curated front per D5 (`pinned`,
`description`, `image`), `tagTermContentConfig` in the registry, the tree
read, `feature {term}`, the `christmas-cookies` seed gains term records.
...
### PR 24b — Site adoption `agent/24b-taxonomy-adopt` 🟡 in progress (← `main`)
